...
ensure_lsphp_modules_recommended() {
  # [ANCHOR:WP_RECOMMENDED_MODULES]
  local php_bin
  local php_version pkg_prefix
  local modules
  local missing=()
  local missing_names=()
  local is_lsws="no"
  ...
  if [ "$is_lsws" != "yes" ]; then
    log_info "未检测到 LiteSpeed/LSPHP 环境，跳过推荐模块检查。"
    return
  fi

  if [ -z "$php_bin" ]; then
    log_warn "未检测到可用的 PHP CLI，跳过推荐模块检查。"
    return
  fi

  php_version="$("$php_bin" -r 'echo PHP_MAJOR_VERSION.PHP_MINOR_VERSION;' 2>/dev/null)" || php_version=""
  if [ -z "$php_version" ]; then
    log_warn "无法识别 PHP 版本，跳过推荐模块安装。"
    return
  fi
  ...
  if [ "${#missing[@]}" -eq 0 ]; then
    log_info "recommended PHP modules already present."
    return
  fi

  if ! command -v apt-get >/dev/null 2>&1; then
    log_warn "未检测到 apt-get，推荐 PHP 模块安装已跳过：${missing[*]}"
    return
  fi

  log_info "检测到缺失推荐 PHP 模块（${missing_names[*]}），尝试安装：${missing[*]}"
  if ! apt-get update -qq; then
    log_warn "apt-get update 失败，推荐模块安装可能无法完成。"
  fi

  if ! apt-get install -y "${missing[@]}"; then
    log_warn "推荐 PHP 模块安装失败，已跳过：${missing[*]}"
    return
  fi

  if systemctl restart lsws >/dev/null 2>&1; then
    log_info "OpenLiteSpeed 已重启以加载推荐模块。"
  else
  ...
  modules="$("$php_bin" -m 2>/dev/null | tr '[:upper:]' '[:lower:]')" || modules=""
  if printf '%s\n' "$modules" | grep -qx "imagick"; then
    log_info "Imagick 模块已加载。"
  else
    log_warn "Imagick 模块仍未加载。"
  fi
  if printf '%s\n' "$modules" | grep -qx "intl"; then
    log_info "Intl 模块已加载。"
  else
    log_warn "Intl 模块仍未加载。"
  fi
}
