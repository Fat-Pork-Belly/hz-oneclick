...
# Guard: must be sourced after REPO_ROOT is defined
if [[ -z "${REPO_ROOT:-}" ]]; then
    echo "Error: REPO_ROOT is not set. Source this library from hz.sh in repo mode."
    return 1 2>/dev/null || exit 1
fi

# Load common colors/log helpers if not already present
