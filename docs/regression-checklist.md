...
- **安装菜单 13（LOMP/LNMP 档位选择）**
  ...
  - 默认基线校验：Akismet/Hello Dolly 已删除（非仅停用）；twenty* 主题仅保留当前启用主题 + 1 个最新默认主题；生成 PHP ini 覆盖文件并仅在变更时重载 lsws；wp-config.php 中 WP_MEMORY_LIMIT/WP_MAX_MEMORY_LIMIT 各只出现一次。
